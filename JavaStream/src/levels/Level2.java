...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        long sum = teachers.stream().filter(t->t.getSubject().equals(Subject.FLUTTER)).mapToInt(Teacher::getSalary).sum();

        /* TO DO 3: Retourner la moyenne des salaires des enseignants dont le nom commence avec a */
        double average = teachers.stream().filter(t -> t.getName().startsWith("a")).mapToDouble(Teacher::getSalary).average().orElse(0);
        ...
        /*First way*/
        teachers.stream() .filter(t -> t.getSubject().equals(Subject.UNITY) && t.getName().startsWith("g"))
                .findFirst()
        ...
        teachers.stream().filter(t-> t.getName().startsWith("s")).skip(1)
                .findFirst()
                .ifPresent(System.out::println);
